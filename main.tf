...
module "kms" {
  ...
  version                 = "1.0.9"
  count                   = var.vpc_flow_log_config.enable_to_cloudwatch ? 1 : 0
...
resource "aws_flow_log" "s3" {
  count                = var.vpc_flow_log_config.enable_to_s3 ? 1 : 0
  log_destination      = var.vpc_flow_log_config.bucket_arn
  log_destination_type = "s3"
  traffic_type         = "ALL"
  vpc_id               = aws_vpc.this.id
  tags = merge(
    {
      Name = "${var.name}-flowlogs-s3"
    },
    var.tags
  )
}
