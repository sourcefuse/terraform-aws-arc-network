...
| <a name="input_vpc_flow_log_config"></a> [vpc\_flow\_log\_config](#input\_vpc\_flow\_log\_config) | Configuration settings for VPC flow logs. | <pre>object({<br>    enable_to_cloudwatch = bool   # Enable VPC flow logs to CloudWatch<br>    retention_in_days    = number # Retention period in CloudWatch<br>    enable_to_s3         = bool   # Enable VPC flow logs to S3<br>    bucket_arn           = string # S3 bucket ARN for VPC flow logs (if enabled)<br>  })</pre> | <pre>{<br>  "bucket_arn": null,<br>  "enable_to_cloudwatch": false,<br>  "enable_to_s3": false,<br>  "retention_in_days": 7<br>}</pre> | no |
